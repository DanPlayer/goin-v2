...
func GetUsername(length int) (username string) {
	if length < 1 {
		length = 1
	}
	mutex.Lock()
	...
	for i := 0; i < length; i++ {
		curIndex := rand.Intn(len(characters))
		username += string(characters[curIndex])
	}
	mutex.Unlock()
	return username
}
...
func EncryptUserPassword(password string) string {
	password = Md5hex(Md5hex("g39jEIs9lbmfCzqs9X" + password))
	return password
}
...
// DateFormat 格式化日期
func DateFormat(date string) (tmp time.Time) {
...
// DayStart 一天的开始时间
func DayStart(date time.Time) (tmp time.Time) {
	tmp = time.Date(date.Year(), date.Month(), date.Day(), 0, 0, 0, 0, date.Location())
	return
}

// DayEnd 一天的最晚时间
func DayEnd(date time.Time) (tmp time.Time) {
	tmp = DayStart(date).AddDate(0, 0, 1)
	return
}

// GetOssLink 获取OSS访问链接
func GetOssLink(source string) string {
	return config.Info.AliYun.OSS.Domain + "/" + source
}

// GetOssPoster 获取缩略图地址
func GetOssPoster(source string)string{
	return config.Info.AliYun.OSS.Domain + "/" + source + "?x-oss-process=image/resize,w_240,m_lfit"
}
